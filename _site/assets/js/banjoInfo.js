// banjoInfo.js (GViz JSONP, no proxy) - build 2026-01-29
// =======================================================
...
const GVIZ = (sheet, tq, cb) =>
  'https://docs.google.com/spreadsheets/d/' + SHEET_ID + '/gviz/tq?' +
  new URLSearchParams({ sheet, tq, tqx: 'responseHandler:' + cb }).toString();
...
function gvizQuery(sheet, tq) {
  return new Promise((resolve, reject) => {
    const cb = `__gviz_${Date.now()}_${Math.floor(Math.random()*1e9)}`;
    const url = GVIZ(sheet, tq, cb);
    ...
    window[cb] = (response) => {
      cleanup();
      // GViz wraps errors here; surface a useful message
      if (response?.status === 'error') {
        const msg = response?.errors?.[0]?.detailed_message || response?.errors?.[0]?.message || 'GViz error';
